...
import com.luckykuang.excel.listener.StudentImportListener;
...
@Service
@AllArgsConstructor
@ExcelImport(ImportBusiness.STUDENT_INFO)
public class StudentImportProcessor implements ImportProcessor {

    private final StudentImportListener studentImportListener;
    ...
    public void process(ImportManagerDTO importManagerDTO) {
        ...
        try {
            ...
            EasyExcelFactory.read(importManagerDTO.getFile().getInputStream(), importManagerDTO.getImportBusiness().getClazz(), studentImportListener)
                    .sheet()
                    .doRead();
