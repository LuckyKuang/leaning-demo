...
@Component
@AllArgsConstructor
public class TeacherImportListener implements ReadListener<TeacherDTO> {

    // 成功数据
    private final List<TeacherDTO> successList = new ArrayList<>();
    // 单次处理条数
    ...
    /**
     * 读取表格内容，每一条数据解析都会来调用
     * @param teacherDTO    one row value. It is same as {@link AnalysisContext#readRowHolder()}
     * @param analysisContext analysis context
     */
    @Override
    public void invoke(TeacherDTO teacherDTO, AnalysisContext analysisContext) {
        if (StringUtils.isBlank(teacherDTO.getName()) || teacherDTO.getAge() == null) {
            log.error("读取错误的行数据:{}", teacherDTO);
            return;
        }
        successList.add(teacherDTO);
        if (successList.size() >= BATCH_COUNT) {
            log.info("读取数据量：{}条", successList.size());
            List<CompletableFuture<Boolean>> futures = new ArrayList<>();
            List<List<TeacherDTO>> lists = ListUtils.partition(successList, 1000);
            for (List<TeacherDTO> list : lists) {
                // 异步处理数据
                CompletableFuture<Boolean> future = CompletableFuture.supplyAsync(() -> {
                    List<Teacher> teachers = list.stream().map(item -> {
    ...
    public void doAfterAllAnalysed(AnalysisContext analysisContext) {
        ...
        if (CollectionUtils.isNotEmpty(successList)) {
            ...
            List<Teacher> teachers = successList.stream().map(item -> {
                Teacher teacher = new Teacher();
                teacher.setName(item.getName());
                teacher.setAge(item.getAge());
                return teacher;
