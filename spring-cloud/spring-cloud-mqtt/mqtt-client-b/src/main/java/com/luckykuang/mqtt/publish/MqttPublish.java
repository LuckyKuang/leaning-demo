...
public class MqttPublish {
     ...
     * @param qos   订阅消息的级别
     */
    public void subscribeMqttMessage(String topic, int qos) {
        try {
            // 建立连接
            subscribeConnect();
            // 以某个消息级别订阅某个主题
     ...
     * @param topic 要订阅的主题
     */
    public void unsubscribeMqttMessage(String topic) {
        try {
            // 建立连接
            subscribeConnect();
            // 取消订阅某个主题
     ...
     * 发布时和emqx服务端建立连接
     */
    private void publishConnect() throws MqttException {
        // 防止重复创建MQTTClient实例
//        try {
...
//        }
     ...
     * 订阅时和emqx服务端建立连接
     */
    protected void subscribeConnect() throws MqttException {
//        try {
        // 防止重复创建MQTTClient实例
        ...
        if (!subscribeClient.isConnected()) {
            ...
            subscribeClient.subscribe("test100",1);
        }
        // 这里的逻辑是如果连接成功就重新连接
        else {
